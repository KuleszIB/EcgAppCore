...
//nieparzystych usuwane są wraz z załamkiem poprzednim.
    ...
    bool flag[N_peak];              //przechowuje inf. czy dany załamek T pozostanie w sygnale
...
//wyznaczania alternansu załamka T
    ...
    sample_counter();
    int N_Twave_sample = STp_min+TpTe_min+1;            //liczba próbek załamka zastępczego
    int N_true_peak = t_peaks.size();                   //liczba załamków T
    ...
    //Wyznaczenie wartości alternansu jako maksymalna różnica między kolejnymi próbkami
    //parzystego i nieparzystego załamka zastępczego
    alt = max(delta);
